...
### Follow-up: headline quality — sentence-case news headlines, not paper titles (2026-08-06, reverted same day)

User feedback (relayed from a fellow user): some headlines read like real news ("A financial complaint classifier shows that simpler models can outperform more complex ones"), others read like a paper/repo title ("A New AI Model for Diverse Movements"). Root cause: the prompt already asked for a "plain-English headline" but didn't explicitly forbid the vague, Title-Case, "A New X enables Y" template, and qwen2.5:7b-instruct falls back to echoing the source title's shape (often literally `ProjectName: Title Case Description`) inconsistently.

- [x] `PROMPT_VERSION` bumped `v6` → `v7`. Headline instruction in `demystifier.py` heavily revised over several iterations: explicit ban on mirroring the source title's shape (colon-prefixed labels), explicit ban on the "A New [Model/Framework/Tool/...] [any verb]s Y" generic-capability template (verb-agnostic, not a fixed list), sentence-case requirement, several bad/good example pairs using the actual feedback examples plus real failures hit during testing.
- [x] Prompt-only fixes plateaued around 100% on template/colon-echoing but only ~35-50% on Title Case specifically, even after 5 rounds of increasingly explicit instruction — measured via a fixed 20-item sample (same items re-tested each round, not fresh random samples, after an early methodology mistake comparing different random samples across rounds). Concluded this is a real 7B-model instruction-following ceiling, not a wording problem, similar to the already-documented article-length ceiling.
- [x] Added a deterministic code-level fix instead of continuing to fight it in-prompt: `_to_sentence_case()` in `demystifier.py`, applied to every `demystify()` call's headline before returning. Preserves the first word, ALL-CAPS tokens (acronyms, alphanumeric like `GPT-4`), and any word with an internal capital (brand names like `ChatGPT`/`PyTorch`); lowercases everything else. Took the same fixed-sample failure rate from ~50-65% to 0/20. Known minor imperfections (acceptable trade-off): hyphenated compounds where both halves are capitalized in the source stay capitalized (e.g. "Tool-Integrated"), and single-word proper nouns/nationality adjectives with ordinary title-case pattern (e.g. "German") can't be distinguished from generic capitalized common words and get lowercased.
- [x] Full corpus (260 items) backfilled under `v7` via repeated `POST /summarize/run?limit=50` batches against the local backend directly. One item (801) hit a JSON-truncation error on first attempt (output cut off at the `num_predict` token cap) — non-fatal per the existing per-item error handling, succeeded on retry in a later batch.
- Verified: real HTTP-endpoint spot checks (not just direct `demystify()` calls) after every prompt/code change, a real backend restart+retest after discovering `uvicorn --reload` silently failed to pick up a `demystifier.py` edit twice in this session (WatchFiles missed the change; a `rm` of an unrelated file did trigger reload, masking the issue) — switched to starting the backend without `--reload` and restarting manually after code changes for the rest of this session to eliminate that whole failure class. Final spot-check of 15 real backfilled headlines: 13/15 clean, 2 with residual minor issues (one colon slip-through, one template slip) — consistent with the measured ~0% casing failure / small residual content-shape failure rate.
- **Reverted the same day, at user's request**, back to `v6`/the original headline prompt (no explicit reason given beyond wanting to go back to how it was). `PROMPT_VERSION` back to `v6` — no backfill needed to revert, since `v6` rows were never deleted, just superseded; `/feed` etc. immediately started serving the pre-existing `v6` data again once the code reverted. The `v7` rows are still sitting in the DB (harmless, just unused) unless cleaned up later. If headline quality comes up again, the prompt wording and the `_to_sentence_case` approach above are both still recoverable from git history (revert commit `8f0a1d3` reverted commit `b3279ee`).
